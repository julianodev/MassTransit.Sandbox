...
namespace MassTransit.Sandbox.Scheduling
{
    public static class SchedulingBus
    {
        ...
        public static async void Start()
        {
            ...
            Console.WriteLine("'5' -> Cancel Scheduling a Message");
                ...
                switch (value)
                {
                    case "1":
                        ...
                        await busControl.GetSendEndpoint(new Uri("rabbitmq://localhost/schedule_notification_queue"))
                            ...
                            {
                                DeliveryTime = DateTime.Now.AddMinutes(3),
                    ...
                    case "2":
                        ...
                        try
                        {
                            ...
                            var hourDelivery = DateTime.Now + TimeSpan.FromMinutes(1);

                            await busControl.CreateMessageScheduler(
                                new Uri("rabbitmq://localhost/notification_queue"))
                    ...
                    case "5":
                        ...
                        var instance = provider.GetService<IMessageScheduler>();
                        var queue = new Uri("rabbitmq://localhost/notification_queue");
                        var id = new Guid("59b00000-5db5-0015-6dbf-08d92f38ee26");
                        await instance.CancelScheduledSend(queue, id);
                        break;
                }
            } while (true);
